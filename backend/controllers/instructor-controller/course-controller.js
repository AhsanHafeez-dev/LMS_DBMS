...
const addNewCourse = async (req, res) => {
  try {
      console.log("handling request in instructor-controller/course-controller on addNewCourse controller")
    ...
    if (saveCourse)
    {
      ...
      res.status(httpCodes.created).json(new ApiResponse(httpCodes.created, saveCourse, " course created successfully"))
    }
        else {
...
const getCourseDetails = async (req, res) => {
  try {
    console.log("handling request in instructor-controller/course-controller on getCourseDetai controller")
    let { id } = req.params;
    id = parseInt(id);
    ...
    if (course) { console.log("course fetched successfully"); res.status(httpCodes.ok).json(new ApiResponse(httpCodes.ok, course, "course details fetched successfully"));}
      ...
      res.status(httpCodes.notFound).json(new ApiResponse(httpCodes.notFound, {}, "course with this id doesnot exist"));
    }
...
const updateCourseById = async (req, res) => {
  try {
    console.log("handling request in instructor-controller/course-controller on updateCourseById controller")
    let { id } = req.params;
    id = parseInt(id);
